...
* 控制无人机起飞，与降落，发布`takeoff_status`表示当前无人机起飞阶段。

此外，在其本身使能的情况下

* 接受`flight_mode_manager`或者是远程电脑发布的`trajectory_setpoint`期望位置。
* 进行位置环的`P`控制，期望位置转化为期望速度。进行速度环的`PID`控制，把期望速度转化为期望加速度，同时进行前馈控制
* 把期望加速度按照多旋翼模型假设，转化为期望三轴推力。
* 把期望三轴推力，转化为期望姿态。
* 发布`vehicle_attitude_setpoint`给`mc_att_control`模块。
...
计算的逻辑是，假设所有的姿态均已达到期望，因为内环速度总是快于外环。

此外,`att_sp.thrust_body[2] = -thr_sp.length();`是为了防止四旋翼在飞行过程中掉高.因为如果直接使用`q_sp`计算的三轴推力，但姿态的变化存在时间，就会直接导致Z轴方向的推力下降，从而导致四旋翼掉高.

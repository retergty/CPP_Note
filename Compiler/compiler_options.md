...
使能一些不完全符合`IEEE`或者`ISO`标准的优化，进一步提高了运行速度。

* 去除对`NaNs`，`Infs`，以及`-0.0`的支持。
* 允许重新排序浮点数的运算。
* 允许使用更快的算法来计算某些数学函数，比如`log`，`exp`，`sin`，`cos`等。
* 去除对次正规数(很小的浮点数)的支持。
...
开启链接时优化，当开启时，在编译源文件为`.o`文件时，编译器便会自动地在`.o`文件内写入相关信息，使得在链接时，这些相关信息可以被链接器读取，并好像把这些`.o`文件放进了一个编译单元内，从而可以实现跨文件优化。
...
不内联所有函数，用于性能分析或者debug时更加清楚。

### `-fno-omit-frame-pointer`

不省略栈帧，用于性能分析或debug时更加清楚。

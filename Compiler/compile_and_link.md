...
链接器在链接时会按照指定的顺序搜索库文件

```bash
g++ main.o -o main -lmylib -lotherlib
```

在这个例子中，如果`mylib`中有符号和`otherlib`中有符号冲突，链接器会优先使用`mylib`中的符号。

### 动态加载顺序

当程序运行时，动态链接器会按照可执行文件中记录的库文件顺序加载动态库。

假如依赖树如下：

* 可执行程序 (Exe) 依赖`so1`和`so2`
* `so1`依赖`A`
* `so2`依赖`B`

程序启动时，Linux 动态链接器 ld.so 是**按层级（广度优先）**来加载库的，加载顺序如下

* **第 0 层**：加载`Exe`本体。
* **第 1 层**：解析`Exe`的依赖，按命令行顺序，先加载`so1`，再加载`so2`
* **第 2 层**：解析上一层库的依赖。先看`so1`的依赖，加载`A`；再看`so2`的依赖，加载`B`。

### 动态库符号表

`Linux`的动态库符号表默认是一个巨大的、全局共享的池子。规则只有一条：先到先得（First One Wins）。

例如上面的依赖树中，如果`A`和`B`都定义了一个符号`foo`，那么无论是`so1`还是`so2`调用这个符号，最终都会调用到`A`中的那个版本。

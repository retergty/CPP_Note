...
条件变量(Condition Variables)是同步方法，作为标志通知线程什么时候共享资源可以使用。相较于互斥量与锁，它提供了一个无需等待(wait-free)的同步方法.`C++`使用定义在头文件`<condition_variable>`的`condition_variable`对象实现条件变量。
...
真共享(true sharing)指的是两个`CPU`内核，一个核尝试去访问并修改相同的内存段，带来另一个核相应的缓存线失效的情况。从而带来了缓存`miss`的情况。

* [What is true sharing?](https://stackoverflow.com/questions/57606409/what-is-true-sharing)

### 伪共享

伪共享(false sharing)是由于缓存线的存在，多线程程序本来不修改同一个变量的值，但是由于这两个变量相互内存靠得很近，在同一个缓存线上，修改了一个变量的值，另一个变量缓存的值也被置为无效，`CPU`只能从内存读取，大幅减慢了`CPU`的运行。
